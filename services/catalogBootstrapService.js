...
const VISUAL_DATASET_DIR = process.env.VISUAL_DATASET_DIR
  ? path.resolve(process.env.VISUAL_DATASET_DIR)
  : path.join(AMZON_BASE_DIR, "final_data", "visual_dataset");
...
function listCandidateCsvs() {
  ...
  if (fs.existsSync(VISUAL_CSV)) return [VISUAL_CSV];
...
async function seedCatalog({ force = false } = {}) {
  ...
  if (csvFiles.length === 0) {
    throw new Error(`No dataset CSV files found in ${DATASET_DIR} or ${VISUAL_DATASET_DIR}`);
  }
  ...
  const adminCreated = await ensureDefaultAdmin();
  return { seeded: true, count: products.length, adminCreated };
}
